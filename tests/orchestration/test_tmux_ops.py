...
def test_tmux_new_session_uses_tmux_cwd_and_preserves_command(
...
def test_tmux_new_session_rejects_empty_command(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    monkeypatch.setattr(
        "agvv.orchestration.tmux_ops.tmux_session_exists", lambda _session: False
...
def test_tmux_session_exists_raises_when_tmux_binary_missing(
...
def test_tmux_pipe_pane_appends_to_log_file(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    captured: dict[str, object] = {}
