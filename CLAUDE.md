...
3. **Checkpoint** — a valid Git commit from the task worktree. Only exists if run exits 0 AND a commit is present; exit 0 without checkpoint downgrades to failed.
...
- **Strict checkpoint semantics.** A run is only "completed" if it produced a readable Git commit.
